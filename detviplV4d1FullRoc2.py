...
if __name__ == '__main__':
    ...
    for i in range(1, 11):
        print("Iteration", i)
        cacheFilename = \
             'output/pvanet_full1/detviplV4d1_test/zf_faster_rcnn_iter_' + \
             str(i) + '0000/detections.pkl'

        if (os.path.exists(cacheFilename)):
            eval_result = []
            for cls in range(1, 2):
                result, fppi_pts, recall_pts = eval_roc_pr(config, 
                    'detviplV4d1_2016_test', cacheFilename, cls)
            ...
            det_file = os.path.join('detviplV4d1-pvanet-ohem-' + \
                  str(i) +  '.pkl')
