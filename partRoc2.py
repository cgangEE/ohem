...
if __name__ == '__main__':
    ...
    for i in range(10, 9, -1):
        print("Iteration", i)
        cacheFilename = \
             'output/pvanet_full1_ohem_DRoiAlignX/part_test/zf_faster_rcnn_iter_' + str(i) + '0000_inference/detections.pkl'

        if (os.path.exists(cacheFilename)):
            eval_result = []
            for cls in range(1, 4):
                result, fppi_pts, recall_pts = eval_roc_pr(config, 
                    'part_2016_test', cacheFilename, cls)
                eval_result.append(result)


            det_id = ['head', 'head-shoulder', 'upper-body']
            plot_curves(i, eval_result, det_id)

            det_file = os.path.join('part-pvanet-ohem-DRoiAlignX-' + str(i) +'.pkl')
