...
def im_detect(net, im, _t, boxes=None):
    ...
    '''
    print("net.blobs['im_info']", net.blobs['im_info'].data)
    im_info = net.blobs['im_info'].data[0]
    im_info[:2] = im_info[:2] / im_info[3:5]
    ...
    return scores, pred_boxes, pred_head, pred_head_shoulder, pred_upper_body
...
def test_net(net, imdb, max_per_image=100, thresh=0.05, vis=False):
    ...
    for i in xrange(num_images):
        ...
        scores, boxes, head, head_shoulder, upper_body = \
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            cls_head_shoulder_dets = \
                    np.hstack((cls_head_shoulder, cls_scores[:, np.newaxis])) \
                    .astype(np.float32, copy=False)
            ...
            cls_upper_body_dets = \
                    np.hstack((cls_upper_body, cls_scores[:, np.newaxis])) \
                    .astype(np.float32, copy=False)
