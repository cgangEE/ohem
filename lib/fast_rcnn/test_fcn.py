...
#    cv2.imwrite(str(idx) +'.png', im)
    ...
    for i in range(14):
        ...
        prob_ij = np.lib.pad(prob, ((50, 0), (50, 0)), 
                'constant', constant_values=0)[:w,:h]
        prob_i = np.lib.pad(prob, ((50, 0), (0, 0)), 
                'constant', constant_values=0)[:w,:]
        prob_j = np.lib.pad(prob, ((0, 0), (50, 0)), 
                'constant', constant_values=0)[:,:h]

        prob = prob - prob_i - prob_j + prob_ij

    #    prob = scipy.signal.fftconvolve(prob, conv, 'same')
