...
def im_detect(net, im, _t, boxes=None, imagename=None, fSubmit=None):
    ...
    cls_prob = net.blobs['cls_prob'].data[0]

    #fSubmit.write(imagename)
    fSubmit.write(imagename.strip().split('/')[-1])
...
def test_net(net, imdb, max_per_image=100, thresh=0.05, vis=False):
    ...
    fSubmit = open('submit.csv', 'w')
    fSubmit.write('image,ALB,BET,DOL,LAG,NoF,OTHER,SHARK,YFT\n')
    ...
    for i in xrange(num_images):
        ...
        im_detect(net, im, _t, box_proposals, imdb.image_path_at(i), fSubmit)
