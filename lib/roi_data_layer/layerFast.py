...
from roi_data_layer.minibatchFast import get_minibatch, get_allrois_minibatch, get_ohem_minibatch
import numpy as np
import yaml
...
class RoIDataLayer(caffe.Layer):
    ...
    def _get_next_minibatch(self):
        ...
        if cfg.TRAIN.USE_PREFETCH:
            ...
            print('len(minibatch_db)', len(minibatch_db))
            print('len(db_inds)', len(db_inds))
...
class OHEMDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.ohem_size = cfg.TRAIN.BATCH_OHEM_SIZE

        idx = 0
        top[idx].reshape(1)
        self._name_to_top_map['loss_cls'] = idx
        idx += 1

        # labels blob: R categorical labels in [0, ..., K] for K foreground
        # classes plus background
        top[idx].reshape(1)
        self._name_to_top_map['loss_bbox'] = idx
        idx += 1
    ...
    def forward(self, bottom, top):
        ...
        cls_score_max = np.max(cls_score, 1)        
        cls_score_max = np.transpose(
                np.vstack( (cls_score_max, cls_score_max) ))
        cls_prob = np.exp(cls_score - cls_score_max)

        cls_prob_sum = np.sum(cls_prob, 1)
        cls_prob_sum = np.transpose(
                np.vstack( (cls_prob_sum, cls_prob_sum)) )
        cls_prob = cls_prob / cls_prob_sum


        flt_min = np.finfo(float).eps
        # classification loss
        loss_cls = [ -1 * np.log(max(x, flt_min)) \
        ...
        if cfg.TRAIN.BBOX_REG:
            ...
            loss_bbox = np.zeros(labels.shape[0])
            for i in np.where(labels > 0 )[0]:
                indices = np.where(bbox_inside_weights[i,:] != 0)[0]
                loss_bbox[i] = sum(bbox_outside_weights[i,indices] * [smoothL1(x) \
                    for x in bbox_inside_weights[i,indices] * (bbox_pred[i,indices] - bbox_target[i,indices])])

        loss_cls = np.array(loss_cls)
        loss = loss_cls + loss_bbox

        hard_inds = get_ohem_minibatch(loss, loss_cls, loss_bbox, 
            rois, labels)

        self.cls_prob = cls_prob

        top[0].data[...] = np.sum(loss_cls[hard_inds]) / len(hard_inds)
        top[1].data[...] = np.sum(loss_bbox[hard_inds]) / len(hard_inds)


    def backward(self, top, propagate_down, bottom):
        bottom[0].diff[...] = np.zeros_like(bottom[0].data, dtype=np.float32)
        bottom[1].diff[...] = np.zeros_like(bottom[1].data, dtype=np.float32)
