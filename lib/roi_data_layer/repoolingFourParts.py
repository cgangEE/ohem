...
class RepoolingLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        pred_head = clip_boxes(pred_head, im_shape)

        pred_head_shoulder = bbox_transform_inv(boxes, head_shoulder_deltas)
        pred_head_shoulder = clip_boxes(pred_head_shoulder, im_shape)

        pred_upper_body = bbox_transform_inv(boxes, upper_body_deltas)
        pred_upper_body = clip_boxes(pred_upper_body, im_shape)


        rois_repool = np.vstack((pred_boxes[:,4:], pred_head[:, 4:],
                pred_head_shoulder[:, 4:], pred_upper_body[:, 4:]))
