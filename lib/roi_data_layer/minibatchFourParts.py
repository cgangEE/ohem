...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        #---------------_cg_ added Head------------------
        gt_head = np.empty((len(gt_inds), 4), dtype=np.float32)
        gt_head[:, 0:4] = roidb[0]['head'][gt_inds, :] * im_scales[0]
        blobs['gt_head'] = gt_head


        #---------- _cg_ added Four parts ------------            
        gt_head_shoulder = np.empty((len(gt_inds), 4), dtype=np.float32)
        gt_head_shoulder[:, 0:4] = \
                roidb[0]['head_shoulder'][gt_inds, :] * im_scales[0]
        blobs['gt_head_shoulder'] = gt_head_shoulder

        gt_upper_body = np.empty((len(gt_inds), 4), dtype=np.float32)
        gt_upper_body[:, 0:4] = roidb[0]['upper_body'][gt_inds, :] * im_scales[0]
        blobs['gt_upper_body'] = gt_upper_body
