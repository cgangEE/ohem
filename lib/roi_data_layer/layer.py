...
class RoISoftmaxDataLayer(caffe.Layer):
    ...
    def _get_next_minibatch(self):
        ...
        if cfg.TRAIN.USE_PREFETCH:
            ...
            blobs = get_minibatch(minibatch_db, self._num_classes)
            imagename = minibatch_db[0]['image']
            labelname = imagename.strip().split('/')[-2]

            label = 0
            classList = ['ALB','BET','DOL','LAG','NoF','OTHER','SHARK','YFT']

            for idx, cls in enumerate(classList):
                if labelname.find(cls) != -1:
                    label = idx
                    break

            blobs['label'] = np.array([[label]])
            return blobs
