...
from roi_data_layer.minibatchFourParts import get_minibatch, get_allrois_minibatch, get_ohem_minibatch
import numpy as np
import yaml
...
class RoIDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        if cfg.TRAIN.HAS_RPN:
            ...
            #---------- _cg_ added Head ------------            
            top[idx].reshape(1, 4)
            self._name_to_top_map['gt_head'] = idx
            idx += 1

            #---------- _cg_ added Four parts ------------            
            top[idx].reshape(1, 4)
            self._name_to_top_map['gt_head_shoulder'] = idx
            idx += 1

            top[idx].reshape(1, 4)
            self._name_to_top_map['gt_upper_body'] = idx
            idx += 1
