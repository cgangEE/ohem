...
def get_minibatch(roidb, num_classes):
    ...
    kp = roidb[0]['keypoints'][0]
    
    h = im_blob.shape[2]
    w = im_blob.shape[3]

    label = np.zeros((14, h, w, 1))
    targets = np.zeros((1, 28, h, w))
    reg_inside_weights = np.zeros((1, 28, h, w)) 

    R = cfg.TRAIN.RADIUS

    for i in range(14):
        x, y, z = kp[i * 3: (i + 1) * 3]
        x = int(x * im_scales[0][0])
        y = int(y * im_scales[0][1])

        if z <= 2:
            cv2.circle(label[i], (x, y), R, 1, -1)

            cv2.circle(reg_inside_weights[0,i*2], (x, y), R, 1, -1)
            cv2.circle(reg_inside_weights[0,i*2+1], (x, y), R, 1, -1)
        elif z == 3:
            label[i] = np.ones((h, w, 1)) * -1.0

        targets[0,i*2] = (x - np.multiply(np.ones((h,1)), 
                np.arange(w)[np.newaxis,:])) / R * reg_inside_weights[0,i*2]
        targets[0,i*2+1] = (y - np.multiply(np.arange(h)[:, np.newaxis], 
                np.ones((1,w)))) / R * reg_inside_weights[0,i*2+1]

    label = label.transpose(0, 3, 1, 2)
    blobs['label'] = label
    blobs['reg_targets'] = targets
    blobs['reg_inside_weights'] = reg_inside_weights
    blobs['reg_outside_weights'] = reg_inside_weights.copy()
...
# _cg_ change TRAIN.BATCH_SIZE to BATCH_OHEM_SIZE = 128
    hard_keep_inds = sorted_indices[0:np.minimum(len(loss), cfg.TRAIN.BATCH_OHEM_SIZE)]
