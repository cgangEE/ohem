...
class douyin(imdb):
    def __init__(self, image_set, year):
        imdb.__init__(self, 'douyin_' + image_set)
        self._image_set = image_set
        self._data_path = os.path.join(cfg.DATA_DIR, 'douyin')
    ...
    def getPerson(self, s):
        s = map(float, s.strip().split())
        ret = []

        for i in range(4):
            ret.append(s[i * 5 + 1: i * 5 + 5] + [i + 1])
        return ret
    ...
    def _get_annotation(self, index):
        ...
        head_shoulder = np.zeros((num_objs, 4), dtype=np.uint16)
        upper_body = np.zeros((num_objs, 4), dtype=np.uint16)
        ...
        for i, person in enumerate(persons):
            ...
            for j in xrange(numParts):
                ...
                if j == 0:
                    ...
                    head_shoulder[ix, :] = [x1, y1, x2, y2]
                elif j == 3:
                    upper_body[ix, :] = [x1, y1, x2, y2]
            ix += 1
