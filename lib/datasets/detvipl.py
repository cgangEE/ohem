...
class detvipl(imdb):
    def __init__(self, image_set, year):
        imdb.__init__(self, 'detvipl_' + image_set)
        self._image_set = image_set
        self._data_path = os.path.join(cfg.DATA_DIR, 'detvipl')
    ...
    def _get_annotation(self, index):
        ...
        filename = os.path.join(self._data_path, 'data-label_',  \
                    index[:-4] + '.txt')
    ...
    def gt_roidb(self):
        ...
        '''
        plt.figure()
        plt.hist(self.w, bins)
        plt.title('w')
        plt.figure()
        plt.hist(self.h, bins)
        plt.title('h')
        plt.show()
        '''
        return gt_roidb
