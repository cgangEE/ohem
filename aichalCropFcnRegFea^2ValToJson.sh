...
def main(thresh, box_thresh):
    pkl = 'output/fcnRegFea^2/aichalCrop_val/zf_faster_rcnn_iter_100000_inference/detections.pkl'
    with open(pkl, 'rb') as fid:
        kps = cPickle.load(fid)


    pkl = './data/aichal/aichalClusterBoxVal.pkl'
    with open(pkl, 'rb') as fid:
        boxes = cPickle.load(fid)[1]


    imdb = get_imdb('aichal_2017_val')
    ...
    with open('pred_fcn_reg_fea^2_val_' 
...
def drawProbCurve():
    ...
    plt.xlabel('prob')
    plt.ylabel('#boxes')
    plt.savefig('aichalCropFcnRegFea^2ProbHist.png')
