...
if __name__ == '__main__':
    ...
    for i in range(10, 9, -1):
        print("Iteration", i)
        cacheFilename = \
             'output/pvanet_full1_ohem_DRoiAlignX/detviplV5d0_test/zf_faster_rcnn_iter_' + str(i) + '0000_inference/detections.pkl'

        if (os.path.exists(cacheFilename)):
            eval_result = []
            for cls in range(1, 5):
                result, fppi_pts, recall_pts = eval_roc_pr(config, 
                    'detviplV5d0_2016_test', cacheFilename, cls)
            ...
            det_file = os.path.join('detviplV5d0-pvanet-ohem-DRoiAlignX-' + str(i) +'.pkl')
