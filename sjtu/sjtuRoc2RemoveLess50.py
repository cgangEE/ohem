    ...
    for detId in xrange(detAll.shape[1]):
            ...
            if num_gt > 0:
                ...
                num_pos = len(np.where(mask_pos & \
                        (gt['bboxes'][:, 2] - gt['bboxes'][:, 0] >= min_width) & \
...
def plotRecallByIterations(fppiList, recallList):
    ...
    for i in range(len(fppiList[0])):
        ...
        plt.plot(range(1, len(fppiList)+1), recall, label='@fppi = ' + str(fppiList[0][i]), linewidth=2.0)
...
if __name__ == '__main__':
    ...
    for i in range(10, 11):
        print("Iteration", i)
        cacheFilename = \
                'output/pvanet_full1_ohem/sjtu_ia_test/zf_faster_rcnn_iter_' +str(i) + \
                '0000/detections.pkl'
        if (os.path.exists(cacheFilename)):
            eval_result = []
            for cls in range(1, 2):
                result, fppi_pts, recall_pts = eval_roc_pr(config, 
                    'sjtu_ia_2016_test', cacheFilename, cls)
            ...
            det_file = os.path.join('ohemRemov.pkl')
