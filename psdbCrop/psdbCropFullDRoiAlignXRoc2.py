...
if __name__ == '__main__':
    ...
    for i in range(10, 9, -1):
        print("Iteration", i)
        cacheFilename = \
             'output/pvanet_full1_ohem_DRoiAlignX/psdbCrop_test/zf_faster_rcnn_iter_' + str(i) + '0000_inference/detections.pkl'

        if (os.path.exists(cacheFilename)):
            eval_result = []
            for cls in range(1, 2):
                result, fppi_pts, recall_pts = eval_roc_pr(config, 
                    'psdbCrop_2015_test', cacheFilename, cls)
            ...
            det_file = os.path.join('psdbCrop-pvanet-ohem-DRoiAlignX-' + str(i) +'.pkl')
