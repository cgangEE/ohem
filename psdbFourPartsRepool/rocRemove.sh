...
#    plot_pr(eval_result, curve_id)
    plt.savefig('psdbFourPartsRepoolRemove.png')
...
if __name__ == '__main__':
    ...
    for i in range(10, 0, -1):
        print("Iteration", i)
        cacheFilename = \
               'output/pvanet_full1_DRoiAlignX_FourParts_Ohem_RepoolRemove/psdbFourParts_test/zf_faster_rcnn_iter_' + str(i) + \
        ...
        if (os.path.exists(cacheFilename)):
            ...
            det_id = ('pedestrain', 'head', 'head-shoulder', 'upper-body')
            plot_curves(eval_result, det_id)


            det_file = os.path.join('psdbFourParts-Ohem-pvanet-DRoiAlignX-RepoolRemove-' + str(i) +'.pkl')
