...
def eval_roc_pr(config, image_set, det_file, cls):
    ...
    x = (np.sum(precision>0.9))-1
    print('x', x)
    print('det_conf', det_conf[x])
    print('precision', precision[x])
    print('recall', recall[x])
...
if __name__ == '__main__':
    ...
    config.min_width = 0
    config.min_height = 0
    config.eval_type = [0, 1, 2, 3, 4, 5, 6, 7, 8]
    ...
    for i in range(15, 16):
        print("Iteration", i)
        cacheFilename = \
                'output/pvanet_full7/fish_val/zf_faster_rcnn_iter_' +str(i) + \
                '0000/detections.pkl'
        ...
        if (os.path.exists(cacheFilename)):
            ...
            for cls in range(1, 8):
                ...
                result, fppi_pts, recall_pts = eval_roc_pr(config, 
                    'fish_2016_val', cacheFilename, cls)
            ...
            print('MAP', MAP)
            det_id = ('1', '2', '3', '4', '5', '6', '7')
            plot_curves(i, eval_result, det_id)

            det_file = os.path.join('eval_result.pkl')
