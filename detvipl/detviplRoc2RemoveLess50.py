...
def eval_roc_pr(config, image_set, det_file, cls):
    ...
    for detId in xrange(detAll.shape[1]):
            ...
            if num_gt > 0:
                ...
                mask_pos = mask_pos & (gt['bboxes'][:, 2] - gt['bboxes'][:, 0] >= min_width) & \
...
if __name__ == '__main__':
    ...
    for i in range(10, 11):
        print("Iteration", i)
        cacheFilename = \
                'output/pvanet_full1/detvipl_test/zf_faster_rcnn_iter_' +str(i) + \
                '0000/detections.pkl'
        if (os.path.exists(cacheFilename)):
            eval_result = []
            for cls in range(1, 2):
                result, fppi_pts, recall_pts = eval_roc_pr(config, 
                    'detvipl_2016_test', cacheFilename, cls)
            ...
            det_file = os.path.join('detvipl-pvanet-ohem-Removed50.pkl')
