...
void generate_anchors(int base_size,
  ...
  for (int j = 0; j < num_scales; ++j) {
      const Dtype scale_w = (Dtype)0.5 * (scales[j * 2] - (Dtype)1);
      const Dtype scale_h = (Dtype)0.5 * (scales[j * 2 + 1] - (Dtype)1);
...
template <typename Dtype>
void ProposalClusterLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  anchors_shape[0] = scales.size() / 2;
  anchors_shape[1] = 4;


  anchors_.Reshape(anchors_shape);
  generate_anchors(base_size_, &scales[0], scales.size() / 2,
...
template <typename Dtype>
void ProposalClusterLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                       const vector<Blob<Dtype>*>& top)
{
  ...
  for (int n = 0; n < bottom[0]->shape(0); ++n) {
	...
	/*
    LOG_IF(INFO, pre_nms_topn) << "   pre_nms_topn";
    LOG_IF(INFO, num_proposals) << "   num_proposals";
	*/
...
#ifdef CPU_ONLY
STUB_GPU(ProposalClusterLayer);
#endif

INSTANTIATE_CLASS(ProposalClusterLayer);
REGISTER_LAYER_CLASS(ProposalCluster);
