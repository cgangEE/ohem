...
if __name__ == '__main__':
    ...
    for i in range(10, 7, -1):
        print("Iteration", i)
        cacheFilename = \
             'output/pvanet_full1_ohem_DRoiAlignX/ua_test/zf_faster_rcnn_iter_' + str(i) + '0000_inference/detections.pkl'

        if (os.path.exists(cacheFilename)):
            eval_result = []
            for cls in range(1, 2):
                result, fppi_pts, recall_pts = eval_roc_pr(config, 
                    'ua_2016_test', cacheFilename, cls)
            ...
            det_file = os.path.join('ua-pvanet-ohem-DRoiAlignX-0.7-' + str(i) +'.pkl')
